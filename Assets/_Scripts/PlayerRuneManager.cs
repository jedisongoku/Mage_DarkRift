...
public class PlayerRuneManager : MonoBehaviour
{
    ...
    [Header("Rune Stats")]
    [SerializeField] private float damageBoostMultiplier = 0;
    [SerializeField] private float attackSpeedMultiplier = 0;
    [SerializeField] private float dashSpeedMultiplier = 0;
    ...
    void DamageBoost()
    {
        playerCombatManager.PrimarySkillDamage += Mathf.RoundToInt(PlayerBaseStats.Instance.PrimarySkillDamage * damageBoostMultiplier);
    }

    void AttackSpeedBoost()
    {
        playerCombatManager.PrimarySkillCooldown -= PlayerBaseStats.Instance.PrimarySkillCooldown * attackSpeedMultiplier;
    }

    void DashReducedCooldown()
    {
        playerCombatManager.SecondarySkillCooldown -= PlayerBaseStats.Instance.SecondarySkillCooldown * dashSpeedMultiplier;
    }
